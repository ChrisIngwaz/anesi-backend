...
app.post("/confirmar-pago", async (req, res) => {
    ...
    try {
      ...
      if (response.data.transactionStatus === 'Approved') {
        ...
        // 1. Actualizamos suscripción y recuperamos datos del usuario (incluyendo el mentor)
        const { data: users, error: updateError } = await supabase.from('usuarios')
          ...
          .eq('email', email)
          .select(); // IMPORTANTE: Recupera la fila actualizada

        const user = (users && users.length > 0) ? users[0] : null;

        // 2. DISPARO A MAKE: Solo si el pago es exitoso y existe un referido
